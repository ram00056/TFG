...
const CompararPolaridad = () => {
    const { index, coleccionComparar } = useParams();  // index de la colección que se le pasa por parámetro

    //Valores para colección Elegida
    const [vPositiva1, setVPositiva1] = useState([[], [], []]);
    const [vNegativa1, setVNegativa1] = useState([[], [], []]);
    const [vNeutra1, setVNeutra1] = useState([[], [], []]);

    const [loaded1, setLoaded1] = useState(false); // Para evitar hacer peticiones GET constantemente

    if (!loaded1) {
        fetch(`http://localhost:5000/colecciones/${index}/${coleccionComparar}/polaridad`)
            ...
            .then(data => {
                ...
                setVPositiva1(procesarAtributo(data.v_positiva));
                setVNegativa1(procesarAtributo(data.v_negativa));
                setVNeutra1(procesarAtributo(data.v_neutra));
                setLoaded1(true);
            })
    ...
    const DatosTabla = [
        CrearValoresTabla('Valor Positivo', vPositiva1),
        CrearValoresTabla('Valor Negativo', vNegativa1),
        CrearValoresTabla('Valor Neutro', vNeutra1)
    ];
...
export default CompararPolaridad;