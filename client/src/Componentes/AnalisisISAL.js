...
const AnalisisISAL = () => {
    ...
    const [valorISAL, setValorISAL] = useState('');
    const [loaded, setLoaded] = useState(false);

    if (!loaded) {
        fetch(`http://localhost:5000/colecciones/${index}/VerISAL`)
            .then(response => response.json())
            .then(data => {
                setNombre(data.nombre_c);
                setValorISAL(data.valor_isal);
                setLoaded(true);
            })
    ...
    const toCSV = () => {
        ...
        // Datos de la característica "ISAL"
        const data = {
            valorISAL: getFormattedValues(valorISAL) // Suponiendo que 'bdlase' es una variable que contiene todos los valores
        };
        ...
        link.setAttribute('download', 'ISAL.csv');
        document.body.appendChild(link); // Requerido para Firefox
        link.click();
    };

    // Función para obtener un valor específico de valorISAL según su posición
    const getValor = (posicion) => {
        if (valorISAL.trim() !== '') {
            const partes = valorISAL.split('/');
    ...
    return (
        <div style={{ textAlign: 'center', padding: '20px' }}>
            ...
            <h1 style={{ textAlign: 'left', marginLeft: '50px', textDecoration: 'underline' }}>ISAL</h1>
            ...
            <div style={{
                ...
                <h3 style={{ fontSize: '20px' }}>Resumen para la Característica "ISAL"</h3>
                <div style={{ textAlign: 'center', margin: '20px auto', width: '80%' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #ccc' }}>
                        <tbody>
                            ...
                            <tr>
                                <td style={{ padding: '10px', borderRight: '1px solid #ccc', fontWeight: 'bold' }}>ISAL</td>
