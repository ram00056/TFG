...
const CompararLemas = () => {
    ...
    const [n_lemas_u1, setNLemasU1] = useState([[], [], []]);
    const [m_caracteres1, setMCaracteres1] = useState([[], [], []]);


    const [loaded1, setLoaded1] = useState(false); // Para evitar hacer peticiones GET constantemente

    if (!loaded1) {
        fetch(`http://localhost:5000/colecciones/${index}/${coleccionComparar}/lemas`)
            ...
            .then(data => {
                ...
                setNLemasU1(procesarAtributo(data.n_lemas_u));
                setMCaracteres1(procesarAtributo(data.m_caracteres));
                setLoaded1(true);
            })
    ...
    const DatosTabla = [
        CrearValoresTabla('Número de Lemas Únicos', n_lemas_u1),
        CrearValoresTabla('Media de Caracteres', m_caracteres1),
    ];
