...
const AnalisisEmofinder = () => {
    ...
    const [vEmofinder, setVEmofinder] = useState(''); // Estado para almacenar el valor de Emofinder
    const [loaded2, setLoaded2] = useState(false); // Para evitar hacer peticiones GET constantemente

    if (!loaded2) {
        fetch(`http://localhost:5000/colecciones/${index}/VerEmofinder`)
            .then(response => response.json())
            .then(data => {
                setNombre(data.nombre_c);
                setVEmofinder(data.v_emofinder);
                setLoaded2(true);
            })
    ...
    const toCSV = () => {
        ...
        // Datos de la característica "emofinder"
        const data = {
            Emofinder: getFormattedValues(vEmofinder) // Suponiendo que 'bdlase' es una variable que contiene todos los valores
        };
    
        // Define las filas del CSV
        const rows = [
            ["Característica", "Mínimo", "Máximo", "Varianza", "Moda", "Media"],
            ["Emofinder", ...data.Emofinder]
        ];
        ...
        link.setAttribute('download', 'emofinder.csv');
        document.body.appendChild(link); // Requerido para Firefox
        link.click();
    };

    // Función para obtener un valor específico de v_emofinder según su posición
    const getValor = (posicion) => {
        if (vEmofinder.trim() !== '') {
            const partes = vEmofinder.split('/');
    ...
    return (
        <div style={{ textAlign: 'center', padding: '20px' }}>
            ...
            <h1 style={{ textAlign: 'left', marginLeft: '50px', textDecoration: 'underline' }}>Análisis de Emofinder</h1>
            ...
            <div style={{
                ...
                <h3 style={{ fontSize: '20px' }}>Resumen para la Característica "EMOFINDER"</h3>
                <div style={{ textAlign: 'center', margin: '20px auto', width: '80%' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #ccc' }}>
                        <tbody>
                            ...
                            <tr>
                                <td style={{ padding: '10px', borderRight: '1px solid #ccc', fontWeight: 'bold' }}>EMOFINDER</td>
...
export default AnalisisEmofinder;
