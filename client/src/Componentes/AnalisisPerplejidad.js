...
const AnalisisPerplejidad = () => {
    const { index } = useParams();
    const [nombre, setNombre] = useState('');
    const [valorPerplejidad, setValorPerplejidad] = useState('');
    const [loaded2, setLoaded2] = useState(false);

    if (!loaded2) {
        fetch(`http://localhost:5000/colecciones/${index}/VerPerplejidad`)
            .then(response => response.json())
            .then(data => {
                setNombre(data.nombre_c);
                setValorPerplejidad(data.valor_perplejidad);
                setLoaded2(true);
            })
    ...
    const toCSV = () => {
        ...
        const data = {
            valorPerplejidad: getFormattedValues(valorPerplejidad) // Suponiendo que 'bdlase' es una variable que contiene todos los valores
        };
    
        // Define las filas del CSV
        const rows = [
            ["Característica", "Mínimo", "Máximo", "Varianza", "Moda", "Media"],
            ["Perplejidad", ...data.valorPerplejidad]
        ];
        ...
        link.setAttribute('download', 'Perplejidad.csv');
        document.body.appendChild(link); // Requerido para Firefox
        link.click();
    };

    // Función para obtener un valor específico de valorPerplejidad según su posición
    const getValor = (posicion) => {
        if (valorPerplejidad.trim() !== '') {
            const partes = valorPerplejidad.split('/');
    ...
    return (
        <div style={{ textAlign: 'center', padding: '20px' }}>
            ...
            <h1 style={{ textAlign: 'left', marginLeft: '50px', textDecoration: 'underline' }}>Perplejidad</h1>
            ...
            <div style={{
                ...
                <h3 style={{ fontSize: '20px' }}>Resumen para la Característica "Perplejidad"</h3>
                <div style={{ textAlign: 'center', margin: '20px auto', width: '80%' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #ccc' }}>
                        <tbody>
                            ...
                            <tr>
                                <td style={{ padding: '10px', borderRight: '1px solid #ccc', fontWeight: 'bold' }}>Perplejidad</td>
                                <td style={{ padding: '10px' }}>
                                    <table style={{ width: '130%', borderCollapse: 'collapse' }}>
                                        <tbody>
                                            <tr>
                                                <td style={{ padding: '10px', fontWeight: 'bold' }}>Minimo</td>
                                                <td style={{ padding: '10px', textAlign: 'left' }}>{getValor(1)}</td>
                                            </tr>
                                            <tr>
                                                <td style={{ padding: '10px', fontWeight: 'bold' }}>Maximo</td>
                                                <td style={{ padding: '10px', textAlign: 'left' }}>{getValor(2)}</td>
                                            </tr>
                                            <tr>
                                                <td style={{ padding: '10px', fontWeight: 'bold' }}>Varianza</td>
                                                <td style={{ padding: '10px', textAlign: 'left' }}>{getValor(3)}</td>
                                            </tr>
                                            <tr>
                                                <td style={{ padding: '10px', fontWeight: 'bold' }}>Moda</td>
                                                <td style={{ padding: '10px', textAlign: 'left' }}>{getValor(4)}</td>
                                            </tr>
                                            <tr>
                                                <td style={{ padding: '10px', fontWeight: 'bold' }}>Media</td>
                                                <td style={{ padding: '10px', textAlign: 'left' }}>{getValor(5)}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </td>
...
export default AnalisisPerplejidad;
