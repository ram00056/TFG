...
const CompararEmociones = () => {
    const { index, coleccionComparar } = useParams();  // index de la colección que se le pasa por parámetro
    
    //Valores para colección Elegida
    const [otros1, setOtros1] = useState([[], [], []]);
    const [alegria1, setAlegria1] = useState([[], [], []]);
    const [tristeza1, setTristeza1] = useState([[], [], []]);
    const [ira1, setIra1] = useState([[], [], []]);
    const [sorpresa1, setSorpresa1] = useState([[], [], []]);
    const [asco1, setAsco1] = useState([[], [], []]);
    const [miedo1, setMiedo1] = useState([[], [], []]);


    const [loaded1, setLoaded1] = useState(false); // Para evitar hacer peticiones GET constantemente

    if (!loaded1) {
        fetch(`http://localhost:5000/colecciones/${index}/${coleccionComparar}/emociones`)
            ...
            .then(data => {
                ...
                setOtros1(procesarAtributo(data.otros));
                setAlegria1(procesarAtributo(data.alegria));
                setTristeza1(procesarAtributo(data.tristeza));
                setIra1(procesarAtributo(data.ira));
                setSorpresa1(procesarAtributo(data.sorpresa));
                setAsco1(procesarAtributo(data.asco));
                setMiedo1(procesarAtributo(data.miedo));
    ...
    const DatosTabla = [
        CrearValoresTabla('Otros', otros1),
        CrearValoresTabla('Alegría', alegria1),
        CrearValoresTabla('Tristeza', tristeza1),
        CrearValoresTabla('Ira', ira1),
        CrearValoresTabla('Sorpresa', sorpresa1),
        CrearValoresTabla('Asco', asco1),
        CrearValoresTabla('Miedo', miedo1)      
    ];
...
export default CompararEmociones;