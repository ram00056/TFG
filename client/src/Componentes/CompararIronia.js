...
const CompararIronia = () => {
    ...
    if (!loaded1) {
        fetch(`http://localhost:5000/colecciones/${index}/${coleccionComparar}/ironia`)
            ...
            .then(data => {
                ...
                setValorNo1(procesarAtributo(data.valor_no));
                setValorSi1(procesarAtributo(data.valor_si));
                setLoaded1(true);
            })
    ...
    const DatosTabla = [
        CrearValoresTabla('No Ironia', valorNo1),
        CrearValoresTabla('Ironia', valorSi1),
    ];
        ...
        <div style={{ textAlign: 'center', padding: '20px' }}>
            ...
            <div style={{ padding: '20px', overflow: 'auto', maxWidth: '100%', maxHeight: '600px' }}>
                <table style={{ borderCollapse: 'collapse', width: '100%', height: '100%' }}>
                    ...
                    <thead>
                    <th style={{ border: '1px solid black', padding: '5px', fontSize: '12px', width: '20%', height: '100%' }}>Característica</th>
                    <th style={{ border: '1px solid black', padding: '5px', fontSize: '12px', width: '26%', height: '100%' }}>Test de Normalidad</th>
                    <th style={{ border: '1px solid black', padding: '5px', fontSize: '12px', width: '26%', height: '100%' }}>Test Paramétricos</th>
                    <th style={{ border: '1px solid black', padding: '5px', fontSize: '12px', width: '26%', height: '100%' }}>Test No Paramétricos</th>
                    ...
                    <tbody>
                        {DatosTabla.map((row, index) => (
                            <tr key={index}>
                                ...
                                <td style={{ border: '1px solid black', padding: '0', fontSize: '12px', width: '100%', height: '100%', verticalAlign: 'top', overflow: 'hidden' }}>
                                    {row.values[0]}
                                </td>
                                <td style={{ border: '1px solid black', padding: '0', fontSize: '12px', width: '100%', height: '100%', verticalAlign: 'top', overflow: 'hidden' }}>
                                    {row.values[1]}
